...
.pagination1 a {
  ...
  <div class="sidenav-iki " style="border-right: 8px solid rgba(156,197,187,0.280); box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    ...
    <a href="TT.php"><i class="fas fa-adjust fa-2x fc-white fa-pulse"></i></a>
  ...
  <div class="main-iki">
    ...
    <div class="puso">
      <br>
      <h1 class="mt-1 GP17"><i class="fas fa-snowboarding fa-fw border-r3 bs-1" style="background:#4be2bf; "></i> <b>List Transaksi Terakhir</b></h1>
    </div>
    <!--  -->
    <br>
...
<?php 
$per_hal=10;
$jumlah_record=mysqli_query($connect, "SELECT * from tbl_penjualan2");
...
if(isset($_GET['cari'])){
  ...
  $db1=mysqli_query($connect, "SELECT * from tbl_penjualan2 where   nb_penjualan like '$cari%' OR jumlah_penjualan like '$cari%'");
}else{
  $db1=mysqli_query($connect, "SELECT * from tbl_penjualan2 limit $start, $per_hal");
}
?>
...
<div class="puso">
      ...
      <tr>
                    ...
                    <td colspan=2>	<?php 
            $x=mysqli_query($connect, "SELECT sum(subt_penjualan) as total from tbl_penjualan2");	
      ...
      <td colspan=2><b>Rp.<?php echo $_SESSION['bayar'] ?></b></td>
      </tr>
      <tr>
      <th colspan=4>Kembalian</th>
      <td colspan=2><b>Rp.<?php echo number_format($_SESSION['hasil']) ?></b></td>
      </tr>
      </tfoot>
        </table>
  </div>
